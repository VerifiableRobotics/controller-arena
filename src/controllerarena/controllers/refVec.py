...
class refVec:
    ...
    def __init__(self, q_0, controller_flag, dt):
        ...
        # set gains
        self.k_p_u = 0.5  # u indicates it is an position gain. p indicates it is a proportional gain.
        self.k_p_w = 1.5   # w indicates it is an angular gain. p indicates it is a proportional gain.
    ...
    def get_control(self, q, q_d, F):
		# I think that this control law is not a function of the vector field, and that it should
		# work if F(q) changes
		#
		# compute control signal u
        delta_p = q[0:2] - q_d[0:2] # location - location_desired
        self.e_int_w += self.sub_angles(q[2][0],q_d[2][0])*self.dt # accumulate angular error
        self.e_int_u += linalg.norm(delta_p)*self.dt # accumulate position error
        theta = q[2][0]
        ...
        if self.phi_prev == None: # if this is the first pass through the controller, phi_dot = 0
            self.phi_prev = phi
		# end if
        phi_dot = (phi-self.phi_prev)/self.dt
        self.phi_prev = phi

        q_dot = (q-self.q_prev)/self.dt
        self.q_prev = q
